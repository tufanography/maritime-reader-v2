...
//   index           — like full, but NEVER sitemap.xml / sitemaps/**. Use when the
//                     REASON to rebuild is the search index: a full build re-emits
//                     a sitemap capped at ARTICLE_PAGE_LIMIT, which is already
//                     BELOW the visible archive (60000 vs 62975, measured
//                     2026-07-28), so publishing it would shorten the live one.
...
// INDEX mode: a full build, but the sitemap files are LEFT ALONE.
//
// Use it when the reason for rebuilding is the SEARCH INDEX, not the sitemap.
// The base `_pagefind/` index is only regenerated by a full build, so refreshing
// it after a bulk delete means running one — and a full build also re-emits the
// sitemap, which in `full` mode would overwrite the live one.
//
// That overwrite is the risk this mode removes. sitemap.xml.ts:21 and
// sitemaps/[name].xml.ts both call listVisible(ARTICLE_PAGE_LIMIT), the same
// ceiling as the article pages, so any build that reads fewer rows than the
// archive holds emits a SHORTER sitemap. Publishing it hides the difference from
// crawlers — exactly the 2026-06-15 incident, where ~49k articles vanished from
// Google's view. MEASURED 2026-07-28: the ceiling (60,000) is already below the
// visible archive (62,975), so every build today emits a short sitemap.
//
// So: upload the index and the pages, keep the sitemap that is already live.
// Mirror image of `delta`, which keeps the base index and uploads the sitemap's
// siblings.
function inScope(key) {
  if (MODE === 'delta') return !key.startsWith('_pagefind/') && key !== 'sitemap.xml' && !key.startsWith('sitemaps/');
  if (MODE === 'index') return key !== 'sitemap.xml' && !key.startsWith('sitemaps/');
  return true; // full
}
